...
export class VerticalDiffManager {
  ...
  // Listener for user doc changes is disabled during updates to the text document by antalyse
  ...
  clearForFilepath(filepath: string | undefined, accept: boolean) {
    ...
    vscode.commands.executeCommand("setContext", "antalyse.diffVisible", false);
  }
  ...
  async acceptRejectVerticalDiffBlock(
    ...
    // Disable listening to file changes while antalyse makes changes
    this.disableDocumentChangeListener();
  ...
  async streamDiffLines(
    ...
    vscode.commands.executeCommand("setContext", "antalyse.diffVisible", true);
  ...
  async streamEdit(
    ...
    if (existingHandler) {
      ...
      // but extra delta was added for each line generated by antalyse
      // Before adding this back, we need to distinguish between human and antalyse
